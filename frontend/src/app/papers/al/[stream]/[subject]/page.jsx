...
export async function generateMetadata({ params }) {
  ...
  const pageTitle = `Download ${subject} A/L (Advanced Level) Past Papers and Marking Schemes | ${stream} Stream`;
  const pageDescription = `Download ${subject} A/L (Advanced Level) past papers and marking scheme. Includes marking schemes and solutions. Free access to ${stream} stream past papers.`;
  
  return {
    title: pageTitle,
    description: pageDescription,
    keywords: `Download ${subject} A/L (Advanced Level) past papers, Download ${subject} past exam papers, Download ${subject} past test papers, Download ${subject} past model papers, Download ${subject} past revision papers, Download ${subject} past study materials, ${subject}, Advanced Level, AL, past papers, ${stream}, marking scheme, solutions, Sri Lanka education, exam preparation`,
    openGraph: {
      title: pageTitle,
      description: pageDescription,
      type: 'website',
    ...
    alternates: {
      canonical: `${SITE_URL}/papers/al/${params.stream}/${params.subject}`,
    },
  };
}

// This function runs on the server side
async function getPapersData(stream, subject, page = 1) {
  try {
...
export default async function ALSubjectPage({ params, searchParams }) {
  ...
  const data = await getPapersData(params.stream, params.subject, currentPage);
  ...
  const generateStructuredData = () => {
    if (!papers || papers.length === 0) {
      return {
        ...
        "description": `Download and view ${subject} Advanced Level past papers. Includes marking schemes and solutions. Free access to ${stream} stream past papers.`,
        "about": {
          "@type": "EducationalSubject",
          "name": `Download ${subject} A/L (Advanced Level) Past Papers and Marking Schemes`,
          "description": `Download ${subject} A/L (Advanced Level) past papers and marking scheme. Includes marking schemes and solutions. Free access to ${stream} stream past papers.`,
    ...
    const paperList = papers.map(paper => ({
      ...
      "name": `Download ${subject} A/L ${paper.year} ${paper.session} Papers and Marking Scheme ${paper.paperNumber}`,
      "description": `Advanced Level ${subject} past paper from ${paper.year} ${paper.session} session. Medium: ${paper.medium}`,
      "educationalLevel": "Advanced Level",
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-16">
            <h1 className="text-3xl font-bold mb-6">
              Select Language for {subject} A/L (Advanced Level) ({stream} Stream)
            </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
              {languages.map((language) => (
                <Link
                  key={language.code}
                  href={`/papers/al/${params.stream}/${params.subject}/${language.code}`}
                  className="p-6 bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow border-2 border-gray-200 hover:border-blue-500"
                >
          ...
          <div className="mt-16">
            <h2 className="text-3xl font-bold mb-6 text-center">
              Download {subject} A/L (Advanced Level) Past Papers & Marking Schemes ({stream} Stream)
            </h2>
            
            <div className="prose prose-lg mb-8 text-center">
              <p className="text-gray-600">
                Browse all available {subject} Advanced Level past papers for the {stream} stream. 
              ...
              <>
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          <h3 className="text-2xl font-semibold mb-2">
                            {paper.year} {paper.examType} {subject} Past Paper & Marking Scheme ({paper.medium.charAt(0).toUpperCase() + paper.medium.slice(1)} Medium)
                          </h3>
                        ...
                        <Link 
                          href={`/papers/al/${params.stream}/${params.subject}/${paper.medium}/${paper.year}`}
                ...
                <div className="mt-8">
                  <ServerPagination
                    ...
                    baseUrl={`/papers/al/${params.stream}/${params.subject}`}
          ...
          <div className="mt-16 bg-gray-50 p-6 rounded-lg">
            <h2 className="text-2xl font-semibold mb-4">About {subject} Advanced Level Past Papers</h2>
            <div className="prose prose-lg">
              ...
              <p className="mt-4">
                All papers are available for downloading in multiple mediums and include marking schemes where available. 
                ...
                <>
                  {[...new Set(papers.map(paper => paper.medium))].map(medium => (
                    <span key={medium} className="px-3 py-1 bg-purple-100 text-purple-800 rounded-full text-sm">
                      {subject} {medium} medium past papers
                    </span>
                  ))}
                </>
              )}
