// app/ol/[subject]/index.tsx
import { router, useLocalSearchParams } from 'expo-router';
import { ActivityIndicator, ScrollView, StyleSheet, Text, TouchableOpacity, View } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { ListRow } from '@/components/UI';
import { useOLPapersBySubject } from '@/hooks/usePapers';
// import { OL_LANGUAGES, OL_SUBJECTS } from '../../../lib/data';
import data from '@/assets/data/data.json';

export default function OLSubjectLanguageSelection() {
  ...
  // Fetch available papers for this subject
  const { papers, loading: papersLoading, error: papersError } = useOLPapersBySubject(sub);

  if (!subjectMeta) {
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.center}>
          <Text style={styles.error}>Invalid subject.</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  const handlePaperGroupPress = (language: string, year: string) => {
    ...
    router.push(`/ol/${sub}/${language}/${year}`);
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.languageSection}>
          {OL_LANGUAGES.map((item) => (
            <ListRow
              key={item.key}
              title={item.label}
              onPress={() =>
                router.push(`/ol/${sub}/${item.key}`)
              }
            />
          ))}
        </View>
        ...
        <View style={styles.availablePapersSection}>
          ...
          {papersLoading ? (
              ...
              {sortedGroupedPapers.map((group: any, index: number) => (
                ...
                >
                  <Text style={styles.paperGroupTitle}>
                    {group.year} O/L {subjectMeta.label}
                  </Text>
            ...
            <View style={styles.center}>
              <Text style={styles.noPapersText}>No papers available for this subject yet.</Text>
            </View>
          )}
        </View>
      </ScrollView>
