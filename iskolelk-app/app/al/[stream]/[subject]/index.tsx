// app/al/[stream]/[subject]/index.tsx
import { router, useLocalSearchParams } from 'expo-router';
import { ActivityIndicator, ScrollView, StyleSheet, Text, TouchableOpacity, View } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { ListRow } from '@/components/UI';
import { useALPapersByStreamSubject } from '@/hooks/usePapers';
// import { AL_LANGUAGES, AL_STREAMS, AL_SUBJECTS_BY_STREAM } from '../../../../lib/data';
import data from '@/assets/data/data.json';

export default function ALSubjectLanguageSelection() {
  ...
  const { stream, subject } = useLocalSearchParams<{ stream?: string; subject?: string }>();
  ...
  // Fetch available papers for this stream and subject
  const { papers, loading: papersLoading, error: papersError } = useALPapersByStreamSubject(s, sub);

  if (!streamMeta || !subjectMeta) {
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.center}>
          <Text style={styles.error}>Invalid stream or subject.</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  const handlePaperGroupPress = (language: string, year: string) => {
    ...
    router.push(`/al/${s}/${sub}/${language}/${year}`);
  };
...
{/*         
        ...
        <View style={styles.languageSection}>
          {AL_LANGUAGES.map((item) => (
            <ListRow
              key={item.key}
              title={item.label}
              onPress={() =>
                router.push(`/al/${s}/${sub}/${item.key}`)
              }
            />
          ))}
        </View>
        ...
        <View style={styles.availablePapersSection}>
          ...
          {papersLoading ? (
              ...
              {sortedGroupedPapers.map((group: any, index: number) => (
                ...
                >
                  <Text style={styles.paperGroupTitle}>
                    {group.year} A/L {subjectMeta.label}
                  </Text>
            ...
            <View style={styles.center}>
              <Text style={styles.noPapersText}>No papers available for this stream and subject yet.</Text>
            </View>
          )}
        </View>
      </ScrollView>
